...
import { 
  CheckCircle, Clock, Search, AlertCircle, Fingerprint, UploadCloud, 
...
export default function Dashboard({ 
  ...
  // Toggle biometrics action
  const handleToggleBiometrics = async (id: string, enabled: boolean) => {
    try {
      const res = await fetch(`/api/applications/${id}/biometrics`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 text-white">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl space-y-6">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-base font-bold font-display tracking-wide uppercase">All Applications</h3>
              <p className="text-xs text-neutral-400">Select pre-approvals to inspect</p>
            </div>
            
            <button 
              onClick={onRefresh}
              className="p-1.5 hover:bg-neutral-850 rounded-lg text-neutral-400 hover:text-red-500 border border-neutral-800 transition"
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-neutral-950 border border-neutral-800 rounded-xl py-2 pl-9 pr-4 text-xs font-medium focus:outline-none focus:ring-1 focus:ring-red-500"
            />
          </div>
          ...
          <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
            {filteredApps.length > 0 ? (
                  ...
                  <div
                    ...
                    className={`p-3.5 rounded-xl border cursor-pointer transition-all ${
                      isActive 
                        ...
                        : 'border-neutral-800/80 bg-neutral-950/40 hover:border-neutral-700 hover:bg-neutral-950/75'
                    }`}
                  >
                    ...
                    <div className="text-left mt-2.5">
                      <h4 className="text-sm font-semibold truncate text-white">{app.applicantName}</h4>
                      <div className="flex justify-between items-center text-[10px] text-neutral-400 mt-1">
                        <span>{app.carYear} {app.carMake} {app.carModel}</span>
                        <span className="font-semibold text-neutral-300 font-mono">₱{app.loanAmount.toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-3.5 pt-2 border-t border-neutral-800/50 flex justify-between items-center gap-2">
                      <span className="text-[9px] text-neutral-500 font-mono">Last Update: {new Date(app.statusUpdatedAt).toLocaleTimeString()}</span>
                      <button
                        onClick={(e) => handleAdvanceStatus(app.id, e)}
                        disabled={advancingId === app.id}
                        className="text-[9px] font-bold text-red-400 bg-red-950/30 hover:bg-red-950/65 px-2 py-1 rounded border border-red-500/15 flex items-center gap-1 transition"
                      >
              ...
              <p className="text-xs text-neutral-500 text-center py-6">No application entries match constraints.</p>
            )}
          </div>
          ...
          <div className="bg-neutral-950 border border-neutral-850 p-4 rounded-xl text-left space-y-2.5">
            ...
            <button
              ...
              className="text-[10px] py-1.5 px-3 w-full bg-neutral-900 hover:bg-neutral-850 rounded border border-neutral-800 text-neutral-300 font-semibold flex items-center justify-center gap-1.5 transition"
            >
        ...
        {activeApp ? (
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 sm:p-8 shadow-xl text-left relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pt-2">
                <div>
                  ...
                  <h2 className="text-xl sm:text-2xl font-bold font-display text-white">{activeApp.applicantName}’s Financing tracking</h2>
                  <p className="text-xs text-neutral-400 mt-1">Car Loan Portfolio: <span className="text-neutral-200">{activeApp.carYear} {activeApp.carMake} {activeApp.carModel}</span></p>
                </div>
                
                {/* Simulated notifications widget */}
                <div className="bg-neutral-950 border border-neutral-800 px-4 py-2.5 rounded-xl text-right">
                  <span className="text-[9px] text-neutral-500 block font-semibold uppercase">Monthly Obligation</span>
              ...
              <div className="my-8 relative">
                ...
                <div className="absolute top-1/2 left-0 right-0 h-1 bg-neutral-800 -translate-y-1/2 rounded-full hidden md:block" />
                <div 
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-4 relative z-10 text-center md:text-left">
                  {[
                    ...
                    return (
                      <div key={idx} className="flex md:flex-col items-center md:items-start gap-4 md:gap-2 text-left bg-neutral-950 md:bg-transparent p-3 md:p-0 border border-neutral-800 md:border-transparent rounded-xl">
                        ...
                        <div>
                          <h4 className={`text-xs sm:text-sm font-semibold font-display ${isActive ? 'text-red-400' : isCompleted ? 'text-green-400' : 'text-neutral-400'}`}>
                            {nodeLabel}
                          </h4>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 bg-neutral-950 p-4 border border-neutral-850 rounded-xl font-mono text-xs">
                <div>
                ...
                <div>
                  ...
                  <span className="font-semibold text-white">{activeApp.creditScore} Tier</span>
                </div>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              ...
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl space-y-4">
                ...
                {activeApp.status === 'DOCUMENTS_PENDING' && (
                  <div className="p-3 bg-red-950/20 border border-red-500/20 rounded-xl text-[11px] text-red-200 leading-relaxed flex gap-2.5">
                ...
                <div className="space-y-2 mt-2">
                  <div 
                    ...
                    className="border border-dashed border-neutral-800 hover:border-red-500/50 bg-neutral-950/50 p-4 rounded-xl text-center cursor-pointer transition-all"
                  >
                    <input 
                      type="file" 
                  ...
                  {activeApp.documents.length > 0 && (
                    <div className="space-y-1.5 pt-2">
                      <span className="text-[10px] font-bold text-neutral-500 uppercase block">Uploaded verification items</span>
                      {activeApp.documents.map((doc, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-neutral-950 p-2.5 rounded-lg border border-neutral-850 text-xs">
                          <span className="font-mono font-medium max-w-[170px] truncate">{doc.name}</span>
                          <span className="text-[9px] bg-red-950/20 text-red-400 border border-red-500/10 px-1.5 py-0.5 rounded font-mono font-bold uppercase">{doc.type}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {/* Account Privacy & WebAuthn simulation */}
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl space-y-4">
                <div className="flex justify-between items-center pb-2 border-b border-neutral-800">
                  <h3 className="text-sm font-bold font-display uppercase tracking-wide flex items-center gap-1.5">
                    <Fingerprint className="w-4 h-4 text-red-500" /> Account Security Lock
                  </h3>
                  <span className={`text-[9px] font-mono font-bold uppercase px-2 py-0.5 rounded ${
                    activeApp.biometricSecured 
                      ? 'bg-green-950/50 text-green-400 border border-green-500/10' 
                      : 'bg-neutral-950 text-neutral-500 border border-neutral-850'
                  }`}>
                    {activeApp.biometricSecured ? 'Biometric Live' : 'Standard PIN'}
                  </span>
                </div>

                <p className="text-xs text-neutral-400 leading-relaxed">
                  Toggle TouchID/FaceID locking system to bind your credential files dynamically, securing underwriter details from external client inspectors.
                </p>

                <div className="bg-neutral-950 p-3.5 border border-neutral-850 rounded-xl space-y-3.5">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-semibold text-neutral-200">Enable Biometric Login Security</span>
                    <button
                      onClick={() => handleToggleBiometrics(activeApp.id, !activeApp.biometricSecured)}
                      className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                        activeApp.biometricSecured ? 'bg-red-600' : 'bg-neutral-800'
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          activeApp.biometricSecured ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <p className="text-[10px] text-neutral-500 leading-normal">
                    *Biometric locking is fully functional. Simulated tokens utilize native browser key isolation.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl text-left">
              <h3 className="text-sm font-bold font-display uppercase tracking-wide mb-4 pb-2 border-b border-neutral-800">Application Underwriting Event Ledger</h3>
              <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                {activeApp.history.map((hist, idx) => (
                  <div key={idx} className="flex gap-4 items-start relative pb-3 border-l-2 border-neutral-800 pl-4 ml-2 last:border-transparent last:pb-0">
                    {/* Ring dot indicator */}
                    <div className="absolute left-[-5px] top-1.5 w-2 h-2 rounded-full bg-red-500 ring-4 ring-neutral-900" />
                    ...
                    <div className="grow">
                      ...
                      <p className="text-xs font-medium text-neutral-300 mt-1 leading-relaxed">
                        {hist.description}
                      </p>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-8 bg-neutral-900 border border-neutral-800 rounded-2xl p-12 text-center text-neutral-500">
            <AlertCircle className="w-12 h-12 text-red-500/50 mx-auto mb-4" />
            <h3 className="text-lg font-bold font-display text-white">No active applications found</h3>
            <p className="text-sm text-neutral-400 mt-1">Please configure your loan quote and click Pre-qualify to file a package.</p>
          </div>
        )}
